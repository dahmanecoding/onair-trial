...
// ===== Google OAuth 2.0 for the Google Health API (replaces legacy Fitbit OAuth) =====
...
const SELF_URL = `${SB_URL}/functions/v1/fitbit-oauth`;
const SCOPES = [
...
Deno.serve(async (req) => {
  try {
    ...
    if (!code) {
      return page("Nothing to do here", "This address is the OAuth callback. To connect your data, open the app and press <b>Connect Fitbit</b> in Settings.", 200);
    }
    ...
    fetch(`${SB_URL}/functions/v1/fitbit-sync`, {
      method: "POST",
    ...
    try {
      ...
      return page("Connected!", "Your Google Fitbit data is linked and the first sync is running. You can close this tab and open the app.", 200);
    }
  } catch (e) {
    console.error("fitbit-oauth error", e);
