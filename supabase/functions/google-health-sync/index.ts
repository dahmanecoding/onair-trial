...
async function refreshIfNeeded(t: any): Promise<any> {
  ...
  await db.from("google_health_tokens").update(upd).eq("user_id", t.user_id);
  return { ...t, ...upd };
}
...
async function ensureIdentity(t: any) {
  if (t.google_user_id) return;
  const j = await gh(t, "/users/me/identity");
  const id = j?.healthUserId ?? j?.legacyUserId;
  if (id) await db.from("google_health_tokens").update({ google_user_id: String(id) }).eq("user_id", t.user_id);
}
...
Deno.serve(async (req) => {
  ...
  const { data: tokens } = await db.from("google_health_tokens").select("*");
  ...
  for (let t of tokens) {
    ...
    try {
      ...
      if (mode === "reconcile" || mode === "backfill") {
        ...
        upserted += await ingestExercise(t, start);
        upserted += await ingestHrIntraday(t, today);
        await computeReadiness(t.user_id, dstr(daysAgo(1)), win);
